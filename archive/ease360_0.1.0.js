	 ...
	 (function($) {
	   ...
	   'use strict';
			  ...
			  _.increment;
      			...
      			if(_.responsive.length != 0) {
      				...
      				for(var i=0; i < _.responsive.length; i++){
      					...
      					if(_.responsive[i].framesHighDPI == undefined)  _.responsive[i].framesHighDPI = _.framesHighDPI;	
      					if(_.responsive[i].width == undefined)  _.responsive[i].width = _.width;
      					if(_.responsive[i].height == undefined)  _.responsive[i].height = _.height;	
      					if(_.responsive[i].flex == undefined)  _.responsive[i].flex = {"w" : false, "h" : false};	
      				 ...
      				 $(window).bind('resize._', function() { _.resize() });
         			...
         			_.responsive.unshift(d);  // will add the default node to the front of the array
			   ...
			   if(_.backgroundSize == "cover") {
			   		_.aspectRatio =  {"w" :  _.width/_.height, "h" : 1};
			   		_.windowSize =  {"w" : $( window ).width()  , "h" :  $( window ).height() };
			   }
		}
		 ...
		 ease360Prototype.createImagesSet = function(){
			...
			var _ = this;
			  ...
			  //dpi reference
			   ...
			   if(_.frameDirection < 0)  _.load.frames.reverse();	
		...
		ease360Prototype.refresh = function() {
			...
			var _width = _.el.css("width");  //get our target width
			var _height = _.el.css("height"); //and height	
			...
			_.el.find("canvas").width(_width);  //set our canvas element to be the same
		...
		ease360Prototype.responsiveInit = function(previousActive) {
				...
				//alert(previousActive + " : " + _.responsiveActive ); 
				
				if( previousActive ==  _.responsiveActive ) {
					
					var _width = _.el.width();  //get our target width
					var _height = _.el.height(); //and height	
				...
				if(  _.responsive[previousActive].images.length == 0) {
		       	 ...
		       	 _.images = []; //clear image set out

		      	 _.frames = _.responsive[ _.responsiveActive].frames;
             	 _.framesHighDPI = _.responsive[ _.responsiveActive].framesHighDPI;  //if the exist
				 ...
				 _.loadingFramesGoal = _.responsive[_.responsiveActive].loadingFramesGoal;
		      	 ...
		      	 // if there are images already contained in the set
		      	  ...
		      	  if( !_.responsive[ _.responsiveActive].activated && _.responsive[ _.responsiveActive].framesReference &&  _.responsive[0].activated )  {
						...
						if(_.images[_.timeline.angle].file == undefined) _.timeline.angle = _.physics.position = 0;
						_.angleUpdateFunc();
						_.canvasSettings = _.canvasMath(_.images[_.timeline.angle]);
						_.canvasDraw(_.images[_.timeline.angle]);
		...
		ease360Prototype.preload = function() {
				...
				//_.loadingFramesGoal = _.loadSet[0].totalframes;
				...
				for (var i = _.currentSet.start; i < 360; i += _.currentSet.increment) {
						...
						if ( (_.newframesloaded == _.currentSet.newframes) && _.events.changeFrames) {
							...
							if (!_.events.interactStart) { 
								...
								if (_.totalLoaded >= _.loadingFramesGoal) { 
									...
									_.loadedSet = _.currentSet;
									_.loadcomplete = true;  //we are now completely loaded, lets tween
									_.totalLoaded = _.newframesloaded  = 0;  //reset
					...
					//_.images[i].src = filename + "?_="+(new Date().getTime());  //add date to fix safari's bug issue not loading files for multiple times
		...
		ease360Prototype.initializeEvents = function() {
	         ...
	         $( _.el.selector ).bind("touchstart", function(e){ _.startInteract(e) });
	         $( _.el.selector ).bind("touchmove", function(e){ _.moveInteract(e) });
	         $( _.el.selector ).bind("touchend", function(e){ _.endInteract(e) });
	            	
			 $( _.el.selector ).bind("mousedown", function(e){ _.startInteract(e) });
			 $( _.el.selector ).bind("mousemove", function(e){ _.moveInteract(e) });
			 $( _.el.selector ).bind("mouseup mouseout", function(e){ _.endInteract(e) });
		...
		ease360Prototype.startMove = function(e) {
        	...
        	var _ = this;
			 ...
			 if( !_.events.interactStart) {
	        	...
	        	if( _.preloadSmart  )_.preload();  // only preload more if we have set it 
	        }
		...
		ease360Prototype.moveInteract = function(e) {
	          ...
	          if(  _.events.prevPos.x != null ||  _.events.prevPos.y != null ) {
	              ...
	              if(_.events.touchdirection == null){
	              		...
	              		if (_.events.touchdirection == "left-right"  &&  _.eventDirection == "left-right")  _.startMove(e);	
	              		if (_.events.touchdirection == "up-down"  &&  _.eventDirection == "up-down")  _.startMove(e);	
	              }
	          }
								
			  if (_.events.touchdirection == "up-down" &&  _.eventDirection == "left-right") return;
			  if (_.events.touchdirection == "left-right" &&  _.eventDirection == "up-down") return;	
	         ...
	         // if(differencePos.y >  differencePos.x) return;
	          ...
	          //physics
	          if (_.events.touchdirection == "left-right"  &&  _.eventDirection == "left-right")  _.physics.acceleration += _.events.differencePos.x * _.physics.delta_multiplier;
			  if (_.events.touchdirection == "up-down"  &&  _.eventDirection == "up-down")  _.physics.acceleration += _.events.differencePos.y * _.physics.delta_multiplier;
		...
		ease360Prototype.buildOut = function(){
			...
			var _ = this;
			var contentHolder, docFrag, _canvas;
		    ...
		    var _width = this.el.width();  //get our target width
			var _height = this.el.height(); //and height	
		...
		ease360Prototype.canvasMath = function(img){
			...
			var diffImageRatio, diffRatio;
			var dWidth, dHeight;

			_.canvas.c.height =  (_.el.height() * _.states.factoring);   // scale up when canvas  
			_.canvas.c.width =  (_.el.width()  * _.states.factoring);
			
			_.canvas.c.style.height =  _.el.height() + 'px';     // scale down when canvas  syle to viewing size
			...
			if( _.el.width() < _.el.height()) {
				...
				var diffRatio = _.canvas.c.width/_.canvas.c.height;  //use 
				...
				if(diffRatio  >   imageRatio ) { 
					...
					diffRatio = _.canvas.c.height/_.canvas.c.width;  //flip 
				...
				var diffRatio = _.canvas.c.height/_.canvas.c.width;  //use 
				var imageRatio = _.height / _.width;
				
				if(diffRatio  >   imageRatio ) {

					diffRatio = _.canvas.c.width/_.canvas.c.height;  //flip 
		...
		ease360Prototype.canvasDraw = function(img){
		 ...
		 if(img.src == "") {
			...
			if(  _.backgroundSize == "cover") {
				...
				_.canvas.ctx.drawImage(img, _.canvasSettings.sx,  _.canvasSettings.sy,  _.canvasSettings.sWidth,  _.canvasSettings.sHeight, _.canvasSettings.dx, _.canvasSettings.dy, _.canvasSettings.dWidth, _.canvasSettings.dHeight);  // dWidth and dHeight are 1/factor when using the retina factoring -- scaled should be norm
		...
		ease360Prototype.angleTo = function(angle, time){
             ...
             if(_.timeline.activeTween ) return;  // if we are currently in a tween, then exit
             ...
             _.timeline.activeTween = _.timeline.directTweenActive = true;
              _.states.status = "start";
              _.stateUpdateFunc();
             _.timeline.easeInterval = setInterval(function(){ _.easeOutCubic() }, 33.33);
    	...
    	ease360Prototype.angleStep = function(angle){
         ...
         ease360Prototype.spinOver = function(speed){
             ...
             _.timeline.easeInterval = setInterval(function(){ setAcceleration() }, 10);
             _.events.mouseEventInterval = setInterval(function(){ _.engine() }, 10);
         ...
         ease360Prototype.spinOut = function(){
           ...
           _.timeline.timer = 0;
           //_.states.status = "stop";
           _.stateUpdateFunc(); // notify the user the tween has stopped
           _.timeline.activeTween = false;
         ...
         ease360Prototype.easeOutQuad = function(){  //function easeOutQuad (t, b, c, d) 
           ...
           var _ = this;
            ...
            var result = -_.timeline.difference * t * (t-2) + _.timeline.startValue;    // return c*t*t + b;
            var f =  Math.floor(result) / _.loadedSet.engine ;  // scale the 0-360 value of result into the provided frame range
			var scaledresult = Math.ceil(f) * _.loadedSet.engine;  // then scale back up after applying the Math.Ceil call
        ...
        ease360Prototype.easeOutCubic = function(){   
            ...
            var result = _.timeline.difference *(t*t*t + 1) + _.timeline.startValue;    //return c*(t*t*t + 1) + b;
            var f =  Math.floor(result) / _.loadedSet.engine ;  // scale the 0-360 value of result into the provided frame range
			var scaledresult = Math.ceil(f) * _.loadedSet.engine;  // then scale back up after applying the Math.Ceil call
